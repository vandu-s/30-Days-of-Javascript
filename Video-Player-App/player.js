...
$(document).ready(function() {
    ...
    const createPlaylistCard = (obj, pos) => {
        ...
        mainDiv.addEventListener('click', (e) => {
            ...
            videoPlaySectionData.map((videoItem) => {
                if (obj.id == videoItem.id) {
                    renderVideoData(videoItem);
    ...
    playlistData.map((item, i) => {
        playListWrapper.appendChild(createPlaylistCard(item, i));

    });
    const iFrame = document.getElementById('video-player');
    const viewsCounts = document.getElementById('views-count');
    const videoTitle = document.getElementById('video-title');
    const videoDescription = document.getElementById('video-description');



    const renderVideoData = (data) => {

        iFrame.src = "https://player.vimeo.com/video/" + data.vimeoId;
        viewsCounts.innerHTML = `<span id="">${data.views/1000} k</span> views`;
        videoTitle.innerHTML = data.title;
        videoDescription.innerHTML = data.description;
    }
    renderVideoData(videoPlaySectionData[0]);

    // window.addEventListener("DOMContentLoaded", () => {
    //     renderVideoData(videoPlaySectionData[0]);
    //     // console.log('hallo hiiiii');
    // });
    const removeClass = () => {
        playlistData.map(item => {
            document.getElementById("card" + item.id).classList.remove('active-card');
        });
    }
